...
SELECT * FROM employee WHERE salary = (SELECT MAX(salary) FROM employee where department = 'SW') and department = 'SW';
...
WHERE salary < (SELECT MAX(salary) FROM employee 
where department = 'HR') and department = 'HR');

SELECT name, salary FROM employee WHERE salary > (SELECT AVG(salary) FROM employee);


SELECT name,department,salary from employee
WHERE (department,salary) IN (SELECT department , MAX(salary) from employee where department is not null group by department);


SELECT name,department,age from employee
WHERE (department,age) IN (SELECT department , MIN(age) from employee where department is not null group by department);

SELECT * FROM
employee e WHERE salary = (SELECT MAX(salary) from employee WHERE department = e.department);
...
WITH cte as(
    select department,count(department) AS CNT from employee group by department
)
SELECT department, cnt from cte where cnt =( 
...
select * from hr_table where salary > (select max(salary) from testing_tabl);


--Aggrigat window function
WITH cte AS (
SELECT id, name, salary, department,
ROUND(AVG(salary) OVER(PARTITION BY department),2) as avg_sal
from employee)
SELECT * FROM CTE WHERE salary > avg_sal;

--If you use Aggregate window funtion + Over(Order by)
-- you can fetch the running data
SELECT id, name, date_of_joining, salary,
SUM(salary) OVER (ORDER BY date_of_joining) as runin_sal -- total salary paid by the org as someone new joined according to date
from employee
ORDER BY date_of_joining;

-- where the cost to company exceeds
WITH cte AS(
    SELECT id,name,date_of_joining,salary,
    SUM(salary) OVER (ORDER BY date_of_joining) as ctc
    from EMPLOYEE
    ORDER BY date_of_joining
)
SELECT * FROM cte
