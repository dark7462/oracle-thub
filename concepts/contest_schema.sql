...
--2 joins
select q.q_id, q.name, q.rating, count(*) from
submissions s join question q
ON s.question_id = q.q_id
group by q.q_id, q.name, q.rating;

SELECT q.q_id,q.contest_id,q.q_name , count(*) FROM
submissions s join question q
ON s.question_id = q.q_id
...
SELECT c.c_type, count(s.question_id) FROM
submissions s join question q
ON s.question_id = q.q_id
JOIN CONTEST c
ON c.c_id = q.contest_id
...
SELECT c.c_type, c.c_name, u.name, count(s.QUESTION_ID) FROM
submissions s join question q
ON s.question_id = q.q_id
JOIN CONTEST c
ON c.c_id = q.contest_id
JOIN users u
ON u.u_id = s.user_id
group by c.c_type, c.c_name, u.name;