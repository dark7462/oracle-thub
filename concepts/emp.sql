...
--5
SELECT name, salary, department from employee where gender = 'Male'  AND department = 'SW' OR department = 'Testing' AND salary > 50000;
--6
SELECT id,name,email,date_of_joining FROM employee where date_of_joining IS null;
--7
SELECT * FROM employee WHERE date_of_joining > to_date('01-01-2020','dd-mm-yyyy');
--8
SELECT id, name, gender, department FROM employee WHERE email is null and department is null;
--9
...
AND salary is null;
--11
Select id, name, age, 52 - age as years_of_service_left FROM employee
...
SELECT 
    ...
    salary,
    CASE 
        WHEN salary IS NULL THEN 'Can''t be determined'
...
UPDATE employee SET salary = 
    CASE 
        WHEN department = 'HR' THEN ROUND(salary * 1.10)
        WHEN department = 'SWE' THEN ROUND(salary * 1.20)
        WHEN department = 'Operations' THEN ROUND(salary * 1.15)
        WHEN department = 'Testing' THEN ROUND(salary * 1.08)
        ELSE salary
    END
WHERE salary IS NOT NULL AND department IN ('HR', 'SWE', 'Operations', 'Testing');
--5
...
--Distinct
-- It is used to get distinct values from single column or multiple columns
select distinct department from employee;
select distinct name from employee;
select distinct age from employee;
select distinct department,gender from employee;
--in and not in 
--It is having inclusive and exclusive s=functions to perform the operation
...
AND date_of_joining > TO_DATE('12-07-2019','dd-mm-yyyy');


SELECT name as greeb, salary FROM employee WHERE salary = (SELECT MIN(salary) FROM employee);
SELECT name as amir, salary FROM employee WHERE salary = (SELECT MAX(salary) FROM employee);
...
-- THIS IS THER PRDER YOU NEED TO FOLLOW