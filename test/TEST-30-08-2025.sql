...
--courses where where not any secondry is mapped
select c.course_name from 
courses c left join course_mappings cm on c.id = cm.course_id and cm.role = 'Secondary' 
group by c.course_name having count(cm.id) = 0;
